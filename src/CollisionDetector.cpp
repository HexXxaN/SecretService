...
void CollisionDetector::set_colliders(const GameMap &p_GameMap) {
  std::vector<StaticRectangularObject *> mapColliders =
      p_GameMap.get_StaticRectangularObjects();
  ...
  for (const auto &building : buildings) {
    std::vector<StaticRectangularObject *> components =
        building->get_components();
...
void CollisionDetector::move_enemies(std::vector<Enemy> &p_enemies,
  ...
  for (auto &enemy : p_enemies) {
    ...
    if (Enemy::get_playerPos().x > -1 && Enemy::get_playerPos().y > -1) {
      ...
      // If at least one enemy reached the last known player position, set the
      ...
      if (enemy.get_Timer().get_current_time() -
              enemy.get_Timer().get_start() >=
          enemy.get_movementTime()) {
...
void CollisionDetector::detect_player(Agent *p_player,
                                      std::vector<Enemy> &p_enemies) const {
    ...
    for (auto &enemy : p_enemies) {
      if (enemy.detect_player(playerPos)) {
        ...
        for (auto &collider : m_colliders) {
          // If there's an object between player and enemy, the enemy won't
          // detect the player
          if (detectionRect.x > collider.x - detectionRect.w &&
              detectionRect.x < collider.x + collider.w &&
              detectionRect.y > collider.y - detectionRect.h &&
              detectionRect.y < collider.y + collider.h) {
            obsticleBetweenEntities = true;
            break;
          }
        }

        // If there's no obsticle between player and enemy, the enemies get the
        // player position
...
bool CollisionDetector::is_player_catched(Agent *p_player, 
                                          const std::vector<Enemy> &p_enemies) const {
...
void CollisionDetector::detect_collisions(MovableCircularObject *p_entity,
                                          Point p_prev) const {

  unsigned short int closestX, closestY;
  unsigned short int radius = p_entity->get_radius();
  unsigned short int halfRadius = radius / 2;
  ...
  for (auto &obsticle : m_colliders) {
    ...
    // Checking if the closest point is inside the circle from Pythagorean
    // theorem
    ...
    if (deltaX * deltaX + deltaY * deltaY < radius * radius) {
      ...
      if (p_prev.x <= obsticle.x - halfRadius &&
          p_prev.y >= obsticle.y - radius &&
          p_prev.y <= obsticle.y + obsticle.h + radius)
        // Move to the left edge
        dotCenter.x = obsticle.x - radius;
      // If the player was to the right of an obsticle
      else if (p_prev.x >= obsticle.x + obsticle.w + halfRadius &&
               p_prev.y >= obsticle.y - radius &&
               p_prev.y <= obsticle.y + obsticle.h + radius)
        // Move to the right edge
        dotCenter.x = obsticle.x + obsticle.w + radius;
      // If player was to the top edge of an obsticle
      else if (p_prev.y <= obsticle.y - halfRadius &&
               p_prev.x >= obsticle.x - radius &&
               p_prev.x <= obsticle.x + obsticle.w + radius)
...
inline bool
CollisionDetector::is_on_walkingSurface(const Enemy &p_enemy) const {

  Point currentPosition = p_enemy.get_dotCenter();

  for (auto &walkingSurface : m_walkingSurfaces) {
    if ((currentPosition.x >= walkingSurface.x &&
         currentPosition.x <= walkingSurface.x + walkingSurface.w) &&
        (currentPosition.y >= walkingSurface.y &&
         currentPosition.y <= walkingSurface.y + walkingSurface.h))
      return true;
  }
  return false;
}
